...
def dashboard_page():
    ...
    db = get_db_session()
    interviews = db.query(models.Interview).filter(models.Interview.candidate_id == st.session_state.user['id']).order_all() if hasattr(db.query(models.Interview), 'order_by') else db.query(models.Interview).filter(models.Interview.candidate_id == st.session_state.user['id']).all()
    
    if not interviews:
        st.info("No interviews yet. Click the button above to start your first session!")
    else:
        for itv in interviews:
            with st.expander(f"Session ID: {itv.id} - Date: {itv.start_time.strftime('%Y-%m-%d %H:%M') if itv.start_time else 'N/A'}"):
